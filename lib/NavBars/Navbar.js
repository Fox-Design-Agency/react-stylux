...
export default class Navbar extends React.Component {
    ...
    toggle() {
        ...
        let styledNavbar, use, smdis, show = '';
        const self = this;
            ...
            switch (this.state.box) {
                case "1":
                    switch (this.state.var) {
                        case "1":
                            styledNavbar = [
                                <MediaQuery key={this.state.keys[0]} minWidth={1224}><nav
    ...
    componentDidMount() {
        ...
        let tempkeys = []
        for (let i = 0; i <= (CHILDS.length * 2) + 3; i++) {
        ...
        if (this.state.box === "1" || this.state.box === "2") {
            ...
            if (+this.state.initialCount !== 0) {
                LGDIS = []
                SMDIS = []
                for (let x = 0; x < +this.state.initialCount; x += 1) {
                    LGDIS.push(CHILDS[x])
                }
                for (let i = +this.state.initialCount; i < CHILDS.length; i += 1) {
                    SMDIS.push(CHILDS[i])
                }
                NAVBARLG = LGDIS.map((x, i) => {
                    return <li key={tempkeys[i +3]} style={navbar_navbar_li}>{x}</li>
                })
                NAVBARSM = SMDIS.map((x, i) => {
                    return <li key={tempkeys[i + 3 + CHILDS.length]} style={ham_navbar_li}>{x}</li>
                })
                switch (this.state.box) {
                    case "1":
                        switch (this.state.vars) {
                            case "1":
                                ...
                                styledNavbar = [
                                    <MediaQuery key={tempkeys[0]} minWidth={1224}><nav
                                        style={use.navbar} id={this.state.navid} className={this.state.navClassName}>
                                        <ul style={use.navbar_navbar} id={this.state.navid} className={`${this.state.navClassName} navbar1-navbar socialness`}>
                                            {NAVBARLG}
                                        </ul>
            ...
            if (+this.state.initialCount !== 0) {
                ...
                NAVBARSM = SMDIS.map((x, i) => {
                    return <li key={tempkeys[i + CHILDS.length + 3]} style={ham_navbar_li}>{x}</li>
                })
                smdis = "none"
                use = navbarFunction("1", props, state, self, smdis)
                styledNavbar = [
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (this.state.box === "1" || this.state.box === "2") {
            ...
            if (+this.state.initialCount !== 0) {
                ...
                NAVBARLG = LGDIS.map((x, i) => {
                    return <li key={this.state.keys[i + 3]} style={navbar_navbar_li}>{x}</li>
                })
                NAVBARSM = SMDIS.map((x, i) => {
                    return <li key={this.state.keys[i + CHILDS.length +3]} style={ham_navbar_li}>{x}</li>
                })
                switch (this.state.box) {
                    case "1":
                        switch (this.state.vars) {
